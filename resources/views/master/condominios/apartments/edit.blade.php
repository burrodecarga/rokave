...
@section('content')
<form action="{{route('condominios-apartments.update',[$condominio,$apartment])}}" class="font-weight-bold font-italic row mx-auto w-75 bg-info rounded p-3"
    method="POST">
    @csrf
    @method('PUT')


    <h5 class="col-12">{{$condominio->name}} <hr>Editar Apartamento<hr></h5>
    ...
    <div class="form-group col-md-3">
        ...
        <select name="user_id" class="form-control rounded @error("phone")is-invalid @else border-0 @enderror">
            ...
            <option value="{{$user->id}}" @if($user->id == $apartment->user_id) selected = "selected" @endif >{{$user->name}}</option>
            @endforeach
        </select>
        @error("user_id")
